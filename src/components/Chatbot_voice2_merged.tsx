...
/**
 * LifeBridge – On‑site Chatbot (voice + i18n + dynamic knowledge + accessible mic)
 * ----------------------------------------------------------------
 * - Zachováva funkcionalitu pôvodného Chatbot_voice2 (navigácia, i18n, FAQ, offers)
 * - Z "accessible" verzie pridáva: veľké spodné tlačidlo mikrofónu + stavové hlášky
 * - TTS (predčítavanie) je predvolene ZAPNUTÉ a dá sa okamžite zastaviť (toggle + pri štarte mikrofónu)
 * - Keď zapneš mikrofón, prebiehajúce čítanie sa okamžite zastaví (cancel())
 */
...
const STR = {
  sk: {
    ...
    foundOffer: (title: string, loc?: string) =>
      `Našiel som ponuku: ${title}${loc ? ` v ${loc}` : ""}.`,
    speakBtn: "🎤 Hovoriť",
    listening: "Počúvam...",
    done: "Hotovo!",
  ...
  en: {
    ...
    foundOffer: (title: string, loc?: string) =>
      `I found an offer: ${title}${loc ? ` in ${loc}` : ""}.`,
    speakBtn: "🎤 Speak",
    listening: "Listening...",
    done: "Done!",
...
const FALLBACK_ROUTES: RouteInfo[] = [
  ...
  },
  {
    path: "/login",
    label: "Prihlásenie",
    short: ["login", "prihlasenie", "prihlásenie", "prihlasit", "sign in", "signin"],
...
export default function ChatbotWidget() {
  ...
  // načítanie dynamických znalostí (site map + offers) – voliteľné
  useEffect(() => {
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    // Ak je TTS povolené, pokúsime sa prečítať pozdrav (pre niektoré prehliadače treba interakciu používateľa)
    if (tts) {
      ...
      {open && (
        <div className="w-[400px] max-w-[92vw] h-[620px] flex flex-col rounded-2xl shadow-2xl bg-white/95 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-blue-600 text-white">
            ...
            <div className="flex items-center gap-2">
              {/* TTS toggle (zastaví aj prebiehajúce čítanie) */}
              <button
                onClick={() => {
                  setTts((v) => {
                    const next = !v;
              ...
              {/* Ponecháme aj pôvodnú ikonu mikrofónu v hlavičke */}
              <button
                onClick={toggleMic}
          ...
          {/* Accessible Voice Controls (veľký dolný mikrofón + stav) */}
