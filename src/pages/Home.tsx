  ...
  return (
    <MainLayout>
      <div className="max-w-6xl mx-auto p-8 space-y-12">
        <h1 className="text-3xl font-bold text-center mb-10">🌉 Ponuky používateľov LifeBridge</h1>

        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-semibold">Nadchádzajúce aktivity</h2>
            <Link to="/blog" className="text-blue-600 hover:underline">Zobraziť všetky</Link>
          </div>
          {error ? (
            ...
            <div className="relative">
              <button
                ...
                onClick={() => scrollBy(-1)}
                className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-10 h-10 w-10 items-center justify-center rounded-full bg-white/80 dark:bg-gray-800/80 shadow border border-gray-200 dark:border-gray-700 hover:bg-white"
              >
              ...
              <button
                ...
                onClick={() => scrollBy(1)}
                className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-10 h-10 w-10 items-center justify-center rounded-full bg-white/80 dark:bg-gray-800/80 shadow border border-gray-200 dark:border-gray-700 hover:bg-white"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5"><path d="m8.25 4.5 7.5 7.5-7.5 7.5"/></svg>
              </button>

              <div ref={scrollerRef} className="flex overflow-x-auto gap-4 pb-2 snap-x snap-mandatory">
                {activities.map((a) => (
                  <Link
                    key={a.id_activity}
                    to="/blog"
                    className="min-w-[280px] max-w-[280px] snap-start shrink-0"
                    aria-label={`Otvoriť aktivitu ${a.title}`}
                  >
                ...
                {activities.length === 0 && (
                  <div className="text-gray-500 text-sm">Zatiaľ žiadne aktivity. Vytvor na stránke Blog.</div>
                )}
              </div>
            </div>
          )}
