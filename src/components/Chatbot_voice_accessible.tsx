...
/**
 * LifeBridge – Chatbot (voice + i18n + dynamic knowledge + accessible)
 * (Complete file ready to drop into src/components/Chatbot_voice_accessible.tsx)
 */
...
const STR = {
  ...
  en: {
    ...
    langSet: "Language switched.",
  },
} as const;

const FALLBACK_ROUTES: RouteInfo[] = [
  { path: "/", label: "Domov", short: ["domov", "home", "ponuky", "karty", "lifebridge", "hlavna strana", "úvodná", "offers"], description: "Prehľad ponúk používateľov (karty s titulkom, popisom, autorom, lokalitou a kategóriou)." },
  { path: "/login", label: "Prihlásenie", short: ["login", "prihlasenie", "prihlasenie", "prihlasit", "sign in", "signin"], description: "Formulár na prihlásenie – email + heslo." },
  { path: "/register", label: "Registrácia", short: ["register", "registracia", "registrácia", "sign up", "signup", "vytvorit ucet", "create account"], description: "Formulár na vytvorenie účtu – meno, priezvisko, email, heslo, dátum narodenia." },
  { path: "/profile", label: "Profil", short: ["profil", "moj profil", "môj profil", "account", "účet", "ucet", "profile", "my profile"], description: "Informácie o používateľovi + priestor na tvorbu vlastných ponúk (karty)." },
  { path: "/users", label: "Používatelia", short: ["používatelia", "uzivatelia", "users", "zoznam uzivatelov", "users list"], description: "Zoznam používateľov načítaný z backendu /api/users." },
];

const FALLBACK_FAQ: QA[] = [
  { q: ["kde sa prihlásim", "ako sa prihlásiť", "kde je login", "login", "prihlasenie", "sign in", "where is login"], a_sk: "Na prihlásenie choď na stránku Prihlásenie. Klikni na tlačidlo nižšie alebo použi horné menu.", a_en: "Go to the Login page. Use the button below or the top navigation." },
  { q: ["kde sa zaregistrujem", "ako si vytvorím účet", "registrácia", "sign up", "registration", "create account"], a_sk: "Účet si vytvoríš na stránke Registrácia. Vyplň všetky polia a odošli formulár.", a_en: "Create your account on the Registration page. Fill in all fields and submit the form." },
  { q: ["kde nájdem ponuky", "karty", "domov", "home", "čo je na úvodnej", "offers", "list of offers"], a_sk: "Ponuky zobrazujeme na domovskej stránke. Nájdeš tam karty s titulkom, popisom a autorom.", a_en: "Offers are shown on the Home page as cards with title, description and author." },
  { q: ["kde je môj profil", "profil", "account", "účet", "where is my profile", "profile"], a_sk: "Tvoj profil je na stránke Profil. Odtiaľ vieš tvoriť vlastné ponuky.", a_en: "Your profile is on the Profile page. From there you can create your own offers." },
  { q: ["kde vidím všetkých používateľov", "users", "zoznam účtov", "list of users", "all users"], a_sk: "Zoznam používateľov je na stránke Používatelia (dáta z /api/users).", a_en: "The users list is on the Users page (data from /api/users)." },
];
...
function score(haystack: string, needles: string[]) {
  ...
  for (const n of needles) {
    const nn = norm(n);
    if (nn && h.includes(nn)) sc += nn.length;
  }
  return sc;
}
...
function Bubble({ from, children }: { from: "bot" | "me"; children: React.ReactNode }) {
  ...
  return (
    <div className={`flex ${isBot ? "justify-start" : "justify-end"}`}>
      <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm leading-relaxed shadow-md ${isBot ? "bg-white/90 dark:bg-gray-800 text-gray-800 dark:text-gray-100" : "bg-blue-600 text-white"}`}>
        {children}
      </div>
    </div>
  );
...
const Flag = ({ lang }: { lang: Lang }) => <span className="text-xl align-middle">{lang === "en" ? "🇬🇧" : "🇸🇰"}</span>;
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  ...
                  setMessages((prev) => [
                    ...prev,
                    <Bubble from="bot" key={`lang-${Date.now()}`}>
                      <div>{STR[next].langSet}</div>
                    </Bubble>,
                  ]);
                }}
                className="ml-2 px-2 py-1 rounded bg-white/15 hover:bg-white/25 text-lg"
                aria-label="Switch language"
                title="Switch language"
              >
                <span className="text-xl">{lang === "sk" ? "🇬🇧" : "🇸🇰"}</span>
              </button>
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80">
            <div className="flex flex-col gap-2">
              <button
                ...
                aria-label={STR[lang].speakBtn}
              >
                {STR[lang].speakBtn} {listening ? "⏺" : ""}
              </button>
              ...
              >
                {recStatus === "listening" && STR[lang].listening}
                {recStatus === "done" && STR[lang].done}
                {recStatus === "error" && STR[lang].error}
                {recStatus === "idle" && " "}
              </div>
            </div>
          </div>
