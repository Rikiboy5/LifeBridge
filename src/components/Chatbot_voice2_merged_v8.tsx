...
/**
 * LifeBridge – Chatbot v8 (focused fixes)
 * 1) Mobile responsiveness: stays docked bottom-right (no fullscreen). Width/height adapt to tiny screens
 *    and respect iOS safe-areas; larger touch targets remain.
 * 2) "Reload" when navigating to the SAME route: if target === current, call navigate(0) to remount page.
 * 3) SK voice less robotic: try female/natural Slovak voices first; gentle rate/pitch for calmer reading.
 */
...
function pickVoiceFor(lang: Lang): SpeechSynthesisVoice | null {
  ...
  const prefix = lang === "en" ? "en" : "sk";
  const preferred = lang === "en"
    ? ["Google US English", "Google UK English Female", "Samantha", "Microsoft Aria"]
    : ["Google slovenský", "Google Slovak", "Microsoft Viktoria", "Viktoria", "Zuzana", "Iveta"];
  for (const name of preferred) {
    const v = cachedVoices.find(vo => vo.name.toLowerCase().includes(name.toLowerCase()));
    if (v) return v;
  }
  const langVoices = cachedVoices.filter(v => v.lang && v.lang.toLowerCase().startsWith(prefix));
  if (langVoices.length) {
    const scored = langVoices
      .map(v => ({
        v, s: (v.name.match(/female|google|natural|neural/i) ? 2 : 0) + (v.name.match(/viktoria|zuzana|iveta/i) ? 1 : 0)
      }))
      .sort((a,b) => b.s - a.s);
    return scored[0].v;
  }
...
export default function ChatbotWidget() {
  ...
  const [panelDims, setPanelDims] = useState<{w:number; h:number}>({ w: 390, h: 620 });
  ...
  useEffect(() => {
    const calc = () => {
      ...
      // width: up to 430px; on tiny phones use 92vw
      const w = Math.min(430, Math.max(320, Math.floor(vw * (vw < 500 ? 0.92 : 0.28))));
      // height: up to 680px; on tiny phones use ~70vh
      const h = Math.min(680, Math.max(520, Math.floor(vh * (vh < 760 ? 0.70 : 0.80))));
      setPanelDims({ w, h });
    };
    calc();
  ...
  // Docked (bottom-right) panel – responsive size
