...
/**
 * LifeBridge – Chatbot v3
 * - Hello + route chips instantly reflect language change (rebuild hello bubble).
 * - Stronger styling for language dropdown in header.
 * - "Go to {label}" uses localized route labels in EN/SK.
 * - TTS toggle persisted in localStorage ("chatbot_tts") and respected across navigations.
 * - Cancels speech on route change if TTS is OFF; restores correct icon state from storage.
 */
...
type RouteInfo = {
  path: string;
  label: string; // default label (SK for fallback routes). We'll localize via path map.
  short: string[];
  description: string;
};
...
const FALLBACK_ROUTES: RouteInfo[] = [
  ...
  { path: "/login", label: STR.sk.routes.login, short: ["login","prihlasenie","sign in"], description:"Formulár na prihlásenie – email + heslo." },
  { path: "/register", label: STR.sk.routes.register, short: ["register","registracia","sign up"], description:"Formulár na vytvorenie účtu – meno, priezvisko, email, heslo, dátum narodenia." },
  ...
  { path: "/users", label: STR.sk.routes.users, short: ["používatelia","users"], description:"Zoznam používateľov načítaný z backendu /api/users." },
];
        ...
        <div className="w-[400px] max-w-[92vw] h-[620px] flex flex-col rounded-2xl shadow-2xl bg-white/95 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-blue-600 text-white">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`p-1 rounded ${tts ? "bg-white/15 hover:bg-white/25" : "bg-white/20 hover:bg-white/30"} `}
          ...
          <form onSubmit={onSubmit} className="p-3 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800">
            <div className="flex items-center gap-2">
              ...
              <button type="submit" className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700">
